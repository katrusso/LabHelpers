...
'''
        ...
        this_labs_questions = Question.objects.all()
        
        for question in this_labs_questions:
            self.question_id = question.question_id
            self.question_text = question.question_text
            self.question_hint = question.question_hint
            self.question_topic = question.topic
            lab_data=[self.question_id, self.question_text, self.question_hint]
        
        #lab_data = [question_id, question_text, question_hint]
        return lab_data
'''        
'''
    def get_queryset(self):
        #Return questions associated with a particular lab # in order of question id
        return this_labs_questions   #filter(lab_list.lab_id) #.order_by('question_id')
        #return Question.objects.all().filter(lab_id=lab_id).order_by('question_id')
        
    for question in this_labs_questions:
            this_labs_hints = self.Question.objects.filter('question_hint')
            this_labs_answer = self.Answer.objects.all().filter('question_id')
        
        self.this_labs_topics = Topic.objects.all().filter('lab_id')
        
        for topic in this_labs_topics:
            self.this_labs_coach = Topic.objects.all(),filter('topic_id')
            '''
